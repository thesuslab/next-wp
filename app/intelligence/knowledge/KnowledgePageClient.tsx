...
interface KnowledgePageClientProps {
  initialArticles: KnowledgeEntry[];
}

export function KnowledgePageClient({ initialArticles }: KnowledgePageClientProps) {
  const [articles, setArticles] = useState<KnowledgeEntry[]>(initialArticles);
    ...
    <main className="min-h-screen bg-background text-foreground pt-12 pb-24">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="max-w-3xl mb-12">
          <div
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-data/10 border border-data/30 text-data text-xs font-mono mb-4"
          ...
          {filteredArticles.map((art) => (
            ...
            >
              <div>
                ...
                {/* Title */}
                <h2 className="font-display font-bold text-lg leading-snug group-hover:text-data transition-colors mb-2">
                ...
                {/* Highlighted Quantitative Telemetry snippet if present */}
                {art.data && (
                  <div className="mb-4 p-3 rounded-xl bg-background/80 border border-border/50 text-[11px] font-mono text-muted-foreground space-y-1">
                    <div className="text-foreground/90 font-medium text-[10px] uppercase tracking-wider mb-1">
                      EMPIRICAL TELEMETRY
                    </div>
                    {art.data.annualTemperatureC && (
                      <div>• Baseline Temp: {art.data.annualTemperatureC}°C (Annual Mean)</div>
                    )}
                    {art.data.annualPrecipitationMm && (
                      <div>• Baseline Precip: {art.data.annualPrecipitationMm} mm</div>
                    )}
